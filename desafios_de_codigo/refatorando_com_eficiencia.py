...
def entrada_de_dados(texto):
    # mensagem sobre validar e normalizar as entradas para evitar inconsistências.
    if "entrada de dados" in texto:
      return "Valide e normalize as entradas para evitar inconsistências."

def nomenclatura_significativa(texto):
    # mensagem sobre usar nomes descritivos para variáveis e funções.
    if "nomenclaturas" in texto:
...
def processar_entrada(texto):
    ...
    if texto in opcoes:
        ...
        # Retorne uma mensagem de opção inválida se o texto não estiver nas opções
